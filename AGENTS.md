...
- Runtimes: Python `3.12+`, Node (CI uses Node 20).
...
If linters are missing locally:
```bash
python -m pip install ruff djlint
```

Use `.cmd` shims on Windows PowerShell:
- `npm.cmd ci`
- `npx.cmd eslint app/web/static/js/`
- `npx.cmd stylelint "app/web/static/css/app.css"`
